...
/*
    SHARED OVERRIDE -- CfgExileCustomCode allows only one override per function, so every
    addon that needs the garbage collector to skip something must add its guard HERE
    rather than registering a competing override.

    Guards currently installed:
      1. Claim-Vehicles  -- "claimed"
      2. sovran_zeus     -- "SOVRAN_ZEUS_PERSIST_ID"
*/
...
// sovran_zeus: never collect a persisted Zeus placement
if !((_object getVariable ["SOVRAN_ZEUS_PERSIST_ID", -1]) isEqualTo -1) exitWith {};
