...
class CfgNetworkMessages
{
	...
	// at all -- no amount, no UID, no target. The price is a server-side
	// constant and the identity is resolved from the session. There is nothing
	...
	class withdrawItemRequest { module="system_safex"; parameters[]={"STRING","SCALAR","STRING"}; };
	class withdrawItemResponse { module="system_safex"; parameters[]={"SCALAR","STRING","SCALAR","STRING","ARRAY","ARRAY"}; };
	class hasSafeXRequest { module="system_safex"; parameters[]={}; };
	class hasSafeXResponse { module="system_safex"; parameters[]={"SCALAR","ARRAY","ARRAY"}; };
	class updateMarXetResponse { module="system_safex"; parameters[]={"SCALAR","ARRAY"}; };
	class withdrawVehicleRequest { module="system_safex"; parameters[]={"STRING"}; };
	class withdrawVehicleResponse { module="system_safex"; parameters[]={"SCALAR","STRING","ARRAY","ARRAY"}; };
	...
	{
		module = "system_trading";
		parameters[] = {"SCALAR", "SCALAR", "STRING", "BOOL"};
	};
...
class CfgExileCustomCode 
{
	...
	*/
	...
	// Claim-Vehicles: stops the garbage collector deleting a vehicle a player has
	// claimed. The override adds a single early exit on the "claimed" variable.
	...
	// Spawn Selection by xsSpawn (bambam)
	...
	// Vehicle Crash Loot salvage overrides
	...
	// Vehicle Protection System (VPS) overrides
	...
	// ExileFlagHacking action execute override
	ExileClient_action_execute = "customcode\client\ExileClient_action_execute.sqf";

	// ExileIncomingMissile vehicle creation overrides
...
class CfgInteractionMenus
{
	...
	*/
	...
	class Flag
	{
		...
		class Actions
		{
			...
			*/
			...
			// Exile_Abandon_Territory (MGTDB) -- server side is exile_abandon.pbo.
			// Condition also requires the flag NOT be stolen, otherwise a raider
			// could abandon a territory they do not own.
	...
	class Locker
	{
		...
		class Actions
		{
			class Loadout: ExileAbstractAction
			{
			...
			class SafeX: ExileAbstractAction
			{
				title = "SafeX Storage";
				condition = "player call ExileClient_util_world_isInTraderZone";
...
class CfgXM8 
{
	...
	// XCSV scoreboard. controlID MUST equal the idc of the XM8SlideXcsvScoreboard
	// class further down, or the slide opens blank.
	class xcsvScoreboard
	{
		controlID = 71800;
		appID = "App15";
		title = "Scoreboard";
...
class XM8_App09_Button: RscExileXM8AppButton1x1
{
    ...
    text = "Build Check";
    onButtonClick = "player call ExileClient_build_check";
    resource = "";
};
...
// XCSV admin teleport. resource = "" means "run code, no GUI", which is all
// this needs - the destinations arrive as scroll actions instead of a custom
// dialog, so there are no Rsc controls to maintain.
//
// The button is visible to everyone because XM8 apps are static config, but
// XCSV_fnc_tpMenu gates on UID and refuses for anyone not whitelisted. A player
// can see that an admin tool exists; they cannot use it.
//
...
// XCSV Scoreboard. The first of our apps with a real slide rather than
...
/*
    ...
    Geometry follows the Exile convention used by every other slide in this
    file: (grid - offset) * cell, where cell is 0.025 wide and 0.04 tall. Those
    magic numbers are Exile's, not ours; matching them is what makes the app sit
    correctly inside the phone at any UI scale.

    idc 71800 must match CfgXM8 >> xcsvScoreboard >> controlID.
    71801 is the text body, written by XCSV_fnc_scoreboardFill.
*/
class XM8SlideXcsvScoreboard: RscExileXM8Slide
{
    idc = 71800;
    ...
    class Controls
    {
        ...
        class ScoreboardGroup: RscExileXM8ControlsGroupNoHScrollbars
        {
            ...
            class controls
            {
                class ScoreboardText: RscExileXM8StructuredText
                {
                    idc = 71801;
                    x = 0;
                    y = 0;
...
/*
    XCSV Field Notes slide. Topic list on the left, body on the right.

    idc 71810 must match CfgXM8 >> xcsvNotes >> controlID.
    71811 topic list, 71812 body text.
*/
/*
    XCSV Faction Standing slide. Single body panel - the bars are drawn in
    structured text because there is no progress-bar control here and block
    characters read correctly at any UI scale.

    idc 71820 must match CfgXM8 >> xcsvStanding >> controlID.
*/
class XM8SlideXcsvStanding: RscExileXM8Slide
{
    idc = 71820;
    ...
    class Controls
    {
        ...
        class StandingGroup: RscExileXM8ControlsGroupNoHScrollbars
        {
            ...
            class controls
            {
                class StandingBody: RscExileXM8StructuredText
                {
                    idc = 71821;
                    x = 0;
                    y = 0;
...
class XM8SlideXcsvPolicy: RscExileXM8Slide
{
    idc = 71830;
    ...
    class Controls
    {
        ...
        class BodyGroup: RscExileXM8ControlsGroupNoHScrollbars
        {
            ...
            class controls
            {
                class PolicyBody: RscExileXM8StructuredText
                {
                    idc = 71833;
                    x = 0;
                    y = 0;
...
class XM8SlideXcsvPrices: RscExileXM8Slide
{
    ...
    class Controls
    {
        ...
        class SearchBox: RscExileXM8SearchEdit
        {
            ...
            h = 1 * (0.04);
            text = "";
            tooltip = "Type part of an item name";
            // Re-filter on every keystroke. The index is prebuilt and the row
            // list is capped, so this stays responsive.
            onKeyUp = "call XCSV_fnc_priceSearch";
        };
...
class XM8SlideXcsvNotes: RscExileXM8Slide
{
    idc = 71810;
    ...
    class Controls
    {
        ...
        class NotesGroup: RscExileXM8ControlsGroupNoHScrollbars
        {
            ...
            class controls
            {
                class NotesBody: RscExileXM8StructuredText
                {
                    idc = 71812;
                    x = 0;
                    y = 0;
...
{
	idc = 85161;
	text = "Sell An Item: (WARNING! Listing an item will remove all attatchments on it! They will be lost!)";
...
class cytxtSellPrice: RscExileXM8Edit
{
	...
	w = 8.5 * GUI_GRID_W;
	h = 1 * GUI_GRID_H;
	tooltip = "Amount of pop tabs to sell your item for.";
};
