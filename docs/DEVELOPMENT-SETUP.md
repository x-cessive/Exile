...
Third-party software is **linked, not vendored** — most of it is licence-restricted,
...
### 1. Arma 3 dedicated server — Steam app `233780`
...
> The dedicated-server package ships stripped PBOs — `characters_f.pbo` is 57 MB against
...
- **Server side** — `@ExileServer`, plus the MySQL schema. The complete stock pack is
  archived in this repo at
  `THIS IS FOR 3DEN EDITOR NOT FOR SERVER/ExileServer-1.0.4a.zip`, which is useful as a
  clean baseline to diff a live install against.
- **Client side** — `@Exile`, Steam Workshop item `1487484880`. Required on the server
  *and* by every player.
...
- Exile needs `sql-mode` relaxed — see `Useful Documents/Change Msql Strict Mode.txt`
...
runs with no database at all — with no obvious error. Either run the 32-bit
`arma3server.exe`, or source 64-bit builds of both extensions (or migrate to extDB3).

Requires the Visual C++ redistributables (2013 and 2015–2022).
...
- **Arma 3 Tools** — Steam app `233800`, free. AddonBuilder, CfgConvert, FileBank.
- **Mikero's tools** — ExtractPbo, MakePbo, pboProject, DeRap.
...
Exile server FPS degrades over hours — a restart cycle every 3–4 hours is standard
...
- **M3Editor / m3e_3den** — <https://github.com/maca134/m3e_3den>
- **Exile 3DEN plugin**

Both are archived in `THIS IS FOR 3DEN EDITOR NOT FOR SERVER/`. Load them as client mods
...
| Native loot-drop replacement | `ExileLootDrop/` — C# source, needs a .NET build. **Build 32-bit to match a 32-bit server exe.** |
| Trader/price setup | `Scripts/Trader-Mod` class lists, `Useful Documents/*.xlsx` pricing sheets |
| Player mod distribution | A3Launcher, or a Steam Workshop Collection |
| Performance monitoring | Arma Server Monitor (ASM) by Fred41 — reports server FPS and thread load |
...
| 2304–2306 | UDP | Steam/BattlEye ancillary |
| 3306 | TCP | MySQL — **localhost only** unless you have a reason |

Open `2302–2306/UDP` inbound for the server executable. Note the rule must point at the
executable you actually run — if you switch between the 32-bit and 64-bit server, repoint
it.
...
1. **`-autoInit` is mandatory** for an unattended server. Without it the dedicated server
   boots fully — host created, BattlEye up — and then sits in an empty lobby, never
   initialising the mission, so the database is never touched. It looks like a broken
   install and produces no error.
2. **32-bit vs 64-bit decides more than performance** — see extDB2 above. It also decides
