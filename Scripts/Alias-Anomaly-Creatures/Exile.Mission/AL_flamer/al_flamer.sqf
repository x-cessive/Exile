...
_flamer = _group createUnit ["O_Soldier_VR_F",getMarkerPos _poz_orig_sc, [],0, "NONE"]; 
_flamer setSpeaker "NoVoice";_flamer disableConversation true;
_flamer setcaptive true; 
_flamer addRating -10000;_flamer setBehaviour "CARELESS";_flamer enableFatigue false;_flamer setSkill ["courage", 1]; RemoveAllItems _flamer; removeUniform _flamer; Removevest _flamer; removeHeadgear _flamer; removeAllWeapons _flamer; _flamer unassignItem "NVGoggles"; _flamer removeItem "NVGoggles";_flamer setUnitPos "UP";
_flamer disableAI "ALL"; _flamer setMass 7000;
...
while {alive _flamer} do 
{
	...
	{
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["Exile_Unit_Player",5]; {_x setDamage (getDammage _x + 0.05); _tip = selectrandom ["04","burned","02","03"]; [_x,[_tip,200]] remoteExec ["say3d"]} forEach (_nearflamer-[_flamer]); 
		...
		else {_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["Exile_Unit_Player",5]; {_x setDamage (getDammage _x + 0.05)} forEach (_nearflamer-[_flamer]); [[_flamer],"AL_flamer\flamer_jump_SFX.sqf"] remoteExec ["execvm"]; [_flamer,_tgt_flamer,_cap_flamer] spawn fnc_jump_flamer};
		sleep 1;
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["Exile_Unit_Player",5]; {_x setDamage (getDammage _x + 0.05); _tip = selectrandom ["04","burned","02","03"]; [_x,[_tip,200]] remoteExec ["say3d"]} forEach (_nearflamer-[_flamer]); 
