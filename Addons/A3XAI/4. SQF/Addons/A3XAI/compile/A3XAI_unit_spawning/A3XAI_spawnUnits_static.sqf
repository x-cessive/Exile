...
for "_j" from 1 to (_numGroups - _groupsActive) do {
	...
	if ((_trigger getVariable ["spawnChance",1]) call A3XAI_chance) then {
		...
		_spawnPos = if ((count _locationArray) > 0) then {_locationArray call A3XAI_findSpawnPos} else {[(getPosATL _trigger),random (_patrolDist),random(360),0] call SHK_pos};
	};
	...
	if ((_totalAI > 0) && {(count _spawnPos) > 1}) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["A3XAI Debug: No units spawned for group %1. Added group to respawn queue.",_unitGroup];};
	};
